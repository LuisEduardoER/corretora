...
function formatDirections(gdir, mode) {
  ...
  if (mode == 0) {
    ...
    var headerStr;
    if (addr[gdir.getNumRoutes() - 1] == null) {
      var latLng = gdir.getRoute(gdir.getNumRoutes() - 1).getEndLatLng();
      headerStr = "(" + latLng.lat() + ", " + latLng.lng() + ")";
    } else {
      headerStr = addr[gdir.getNumRoutes() - 1];
    }
    retStr += "\t<tr class='heading'><td class='heading'>"
      + "<div class='centered-directions'><img src='icons/iconr"
...
function loadAtStart(lat, lng, zoom) {
  if (GBrowserIsCompatible()) {
    ...
    alert('Seu browser não é compativel com essa tecnologia.\nPor favor considere uma atualização.');
  }
}
...
function clickedAddAddress() {
  tsp.addAddress(document.address.addressStr.value, addAddressSuccessCallback);
}
...
function addAddressSuccessCallback(address, latlng) {
  if (latlng)
    ...
    alert('Erro no endereço: ' + address);
}
...
function onSolveCallback(myTsp) {
  ...
  pathStr += "<input type='button' value='Latitude e Logitude' onClick='toggle(\"exportData\");'>";
